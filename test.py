...
def test_model(feat_model_path, loss_model_path, part, add_loss, device):
    ...
    # model = torch.load(feat_model_path, map_location="cuda") #if on gpu
    ...
    test_set = ASVspoof2019("LA", "../LAFeatures/",
                            "../DS_10283_3336/LA/ASVspoof2019_LA_cm_protocols/", part,
...
def test(model_dir, add_loss, device):
    ...
    test_model(model_path, loss_model_path, "dev", add_loss, device) # change 'dev' to 'eval' later  

def test_individual_attacks(cm_score_file):
    asv_score_file = os.path.join('../DS_10283_3336',
                                  'LA/ASVspoof2019_LA_asv_scores/ASVspoof2019.LA.asv.dev.gi.trl.scores.txt') # change 'dev' to 'eval' later
