...
class ASVspoof2019(Dataset):
    def __init__(self, access_type, path_to_features, path_to_protocol, part='train', feature='LFCC',
        ...
        protocol = os.path.join(
            self.path_to_protocol, 'ASVspoof2019.'+access_type+'.cm.' + self.part + '.trl.txt')
    ...
    def __getitem__(self, idx):
        ...
        try:
            with open(self.ptf + '/' + filename[-7:] + self.feature + '.pkl', 'rb') as feature_handle:
            ...
            def the_other(train_or_dev):
                ...
                return res
            with open(os.path.join(self.path_to_features, the_other(self.part)) + '/' + filename[-7:] + self.feature + '.pkl', 'rb') as feature_handle:
                feat_mat = pickle.load(feature_handle)
...
if __name__ == "__main__":
    path_to_database = '../DS_10283_3336/'  # if run on GPU
    path_to_features = '../ASVspoof2019LAFeatures/'  # if run on GPU
    path_to_protocol = '../DS_10283_3336/LA/ASVspoof2019_LA_cm_protocols/'
